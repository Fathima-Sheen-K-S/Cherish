...
// Fetch user replies and their statuses
$messagesQuery = $conn->prepare("SELECT m.reply_q1, m.reply_q2, m.comments, m.created_at, m.status, u.name AS receiver_name FROM messages m JOIN users u ON m.receiver_id = u.id WHERE m.sender_id = ?");
      ...
      <ul class="space-y-4">
        <?php while ($message = $messagesResult->fetch_assoc()): ?>
          <li class="bg-white p-4 rounded-lg shadow">
            ...
            <p class="text-gray-600 mt-2"><strong>Reply Q1:</strong> <?= htmlspecialchars($message['reply_q1']) ?></p>
            <p class="text-gray-600 mt-2"><strong>Reply Q2:</strong> <?= htmlspecialchars($message['reply_q2']) ?></p>
            ...
            <p class="mt-2 text-sm font-semibold 
                <?= $message['status'] === 'Accepted' ? 'text-green-600' : ($message['status'] === 'Rejected' ? 'text-red-500' : 'text-gray-500') ?>">
